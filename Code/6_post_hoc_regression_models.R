...
# Any medication regression analyses 
# (complete case and multiple imputation)
...
# 
# Outputs:
#   - Outputs/lineality_plot_lithium_year_[date]: Linearity diagnostic plot for lithium outcome
#   - Outputs/lineality_plot_ad_mono_year_[date]: Linearity diagnostic plot for antidepressant monotherapy outcome
#   - Outputs/lineality_plot_any_med_year_[date]: Linearity diagnostic plot for any medication outcome
#   - Outputs/lithium_regression_combined_[date]: Regression output for lithium models
#   - Outputs/ad_regression_combined_[date]: Regression output for antidepressant monotherapy models
#   - Outputs/any_med_regression_combined_[date]: Regression output for any medication models
#   - Outputs/lithiumAD_anymed_forest_plot_[date]: Faceted forest plot of lithium, antidepressant monotherapy, and any medication regression output (PNG and PDF)
# -----------------------
...
censor_dates <- bipolar_cohort %>%
  ...
  mutate(deathdate = as.Date(deathdate),
         ...
         month_since_diag = ifelse(diff_since_diag >= 0,  # (should all be >= 0)
                                   ceiling(diff_since_diag / 30.42),  # after diagnosis
...
# NOTE: any_med_prescribed derived here before rm() calls, using prescriptions_combs_final
patient_lithium_ad <- prescriptions_combs_final %>%
  group_by(patid) %>%
  summarise(
    lithium_prescribed = as.integer(any(grepl("Lithium", ingredient_combination, ignore.case = TRUE), na.rm = TRUE)),
    ad_only_prescribed = as.integer(any(drug_combination == "AD", na.rm = TRUE)),
    any_med_prescribed = as.integer(
      any(!drug_combination %in% c("None", "Int/Disc", "Censored"), na.rm = TRUE)
    ),
    .groups = "drop"
  )
...
# Now safe to remove both objects
...
model_data <- bipolar_cohort %>%
  ...
  mutate( # recoding missing to 0 (people that did not receive prescriptions within 1y)
    ...
    any_med_prescribed = if_else(is.na(any_med_prescribed), 0L, any_med_prescribed),
  ...
  mutate(across(
    c(lithium_prescribed, ad_only_prescribed, any_med_prescribed),
...
plot_year_linearity <- function(data, outcome_var, outcome_label, title_prefix = NULL) {
  ...
  smoothers <- tribble(
    ...
    "Linear",             "lm",    NULL,                       col_linear, scales::alpha(col_linear, 0.25),    "dashed",
    "Linear + Quadratic", "lm",    y ~ poly(x, 2, raw = TRUE), col_quad,   scales::alpha(col_quad,   0.25),    "dotdash"
  )
...
# Any medication
any_med_table_combined <- generate_combined_table(
  outcome = "any_med_prescribed",
  cc_data = model_data,
  imp_data = model_data_imp,
  vars_to_model = all_vars)

gtsave(any_med_table_combined,
       file = paste0(output_dir, "/any_med_regression_combined_", today(), ".docx"))
...
plot_forest_faceted <- function(model_data_imp, vars_to_model, var_labels,
  ...
  plot_data <- plot_data %>%
    mutate(outcome = case_when(
      ...
      outcome == "any_med_prescribed"  ~ "Any medication",
      TRUE ~ as.character(outcome)
    )) %>%
    mutate(outcome = factor(outcome, levels = c("Lithium", "Antidepressant monotherapy", "Any medication")))
  
  # Create SHARED row numbers (not per outcome)
  plot_data <- plot_data %>%
  ...
  first_outcome <- plot_data %>%
    filter(outcome == first(levels(outcome))) %>%
  ...
  plot_data <- plot_data %>%
    ...
    mutate(show_label = outcome == first(levels(outcome)))
  ...
  p <- ggplot(plot_data, aes(y = shared_row)) +
    ...
    # Reference line
    geom_vline(xintercept = 1, linetype = "dashed", color = "gray50", linewidth = 0.6) +
    # Left-hand labels (first outcome panel only)
    geom_text(
    ...
    # Facet by outcome
    facet_wrap(~ outcome, ncol = 3, scales = "fixed") +
    scale_x_continuous(name = "Odds Ratio (95% CI)", limits = c(plot_min, plot_max), expand = c(0, 0)) +
    scale_color_identity() +
    scale_fill_identity() +
    coord_cartesian(xlim = c(plot_min, plot_max), clip = "off") +
...
# Generate faceted forest plot with all three outcomes
forest_faceted <- plot_forest_faceted(
  model_data_imp = model_data_imp,
  vars_to_model = all_vars,
  var_labels = var_labels,
  outcomes = c("lithium_prescribed", "ad_only_prescribed", "any_med_prescribed"))

# Save as PNG
ggsave(
  filename = paste0(output_dir, "/lithiumAD_anymed_forest_plot_", today(), ".png"),
  plot = forest_faceted,
  width = 60,    # wider to accommodate three panels
  height = 25,
  units = "cm",
  dpi = 300,
  bg = "white")
...
# Save as PDF
ggsave(
  filename = paste0(output_dir, "/lithiumAD_anymed_forest_plot_", today(), ".pdf"),
  plot = forest_faceted,
  width = 60,
  height = 25,
  units = "cm",
...
overall_risk_odds <- map_dfr(
  c("lithium_prescribed", "ad_only_prescribed", "any_med_prescribed"), ~ tibble(
    Outcome = .x,
